...
BEGIN
    ...
    BEGIN TRY
        IF EXISTS (SELECT 1 FROM Categories WHERE CategoryName = @CategoryName AND CategoryID <> @CategoryID)
        BEGIN
        ...
        SELECT @UpdatedCategoryName = CategoryName FROM Categories WHERE CategoryID = @CategoryID;
